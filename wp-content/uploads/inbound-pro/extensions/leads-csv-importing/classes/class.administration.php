...
class Leads_CSV_Processing {
        ...
        <div class='nav-container step-1 active'>
            <form class='csv-file-upload' method="post" enctype="multipart/form-data">
                ...
                <div class="input-group">
					<span class="input-group-btn">
						<span class="btn btn-primary btn-file">
							<?php _e('Browse', 'inbound-pro'); ?>&hellip; <input type="file" name="csv_file"
                ...
                <select name='csv_delimiter' class='form-control select-delimiter'>
                    ...
                    <option value='semicolon'><?php _e('semicolon', 'inbound-pro'); ?></option>
                ...
                <div class='continue-button'>
                    <button type="submit" class="btn btn-primary ladda-button next-1"
                            data-style='expand-right'><?php _e('Next Step (upload CSV)', 'inbound-pro'); ?></button>
                </div>
            </form>
            <br>
        </div>
        ...
        <div class='nav-container step-2 inactive'>
            <form class='field-mapping-rules' method="post">
                <div class='process-button'>
                    <button type="submit" class="btn btn-primary ladda-button next-2"
                            data-style='expand-right'><?php _e('Next Step (Start Importing)', 'inbound-pro'); ?></button>
                </div>
                <h4>Map Columns</h4>
        ...
        <div class='nav-container step-3 inactive'>
            ...
            <div class="progress" style="height:38px;">
                <div id="progressBar" class="progress-bar progress-bar-info progress-bar-striped active"
                     role='progressbar' style="height:38px;padding-top:8px;font-size:21px;"></div>
            </div>
            <br>
    ...
    public static function save_csv_file() {
        ...
        /* Save temp json file */
        $result = self::set_json_store(json_encode(self::array_utf8_encode($csv_data)));
    ...
    public static function get_json_store() {
        $request = wp_remote_get(INBOUND_CSV_IMPORTING_UPLOADS_URLPATH . 'import.json?cachebust=' . rand(0, 9999999));
    ...
    public static function save_mapping_rules() {
        ...
        /* set batch size */
        $csv_data['rows'] = array_chunk($csv_data['rows'], apply_filters('leads/csv-import/batch-limit', 10), true);

        self::set_json_store(json_encode(self::array_utf8_encode($csv_data)));
            ...
            while (($row = fgetcsv($handle, 0, $delimiter)) !== FALSE) {
                ...
                if (!$header) {
                    ...
                    if (count($row) != count($header)) {
                        $difference = count($row) - count($header);
                        $row = array_slice($row, 0, -$difference);
                    }
